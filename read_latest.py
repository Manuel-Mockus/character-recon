...
mat2 = sio.loadmat('dataset_Marin.mat')
data2 = np.transpose(mat2['data'])
label2 = np.array(mat2['label'])
label2 = label2.astype(int)

I2 = [[]for i in range(10)]
#print(len(data))
for i in range(len(data2)):
    I2[label2[0][i]].append(255-data2[i])
...
def testNorm(Tab,Centroids):
    pourcentages=[]
    M = sum(pourcentage(Tab,Centroids,np.inf))/10
    pourcentages.append(M)
    for i in range(1,20):
        k = (pourcentage(Tab,Centroids,i))
        M = sum(k)/10
        pourcentages.append(M)
        print(i)
